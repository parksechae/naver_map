...
function setCarMarker(latitude, longitude) {
    ...
    for (var i = 0; i < 1; i++) {
        ...
        if (i == 0) {
            ...
            path = [
                new naver.maps.LatLng(latitude + 0.0005, longitude + 0.0005),
                new naver.maps.LatLng(latitude + 0.000, longitude + 0.001),
                new naver.maps.LatLng(latitude + 0.000, longitude + 0.001),
                new naver.maps.LatLng(latitude + 0.000, longitude + 0.001),
                new naver.maps.LatLng(latitude + 0.000, longitude + 0.002),
                new naver.maps.LatLng(latitude + 0.001, longitude + 0.002),
                new naver.maps.LatLng(latitude + 0.001, longitude + 0.003),
                new naver.maps.LatLng(latitude + 0.001, longitude + 0.003),
                new naver.maps.LatLng(latitude + 0.002, longitude + 0.002),
...
function setCurrentPosition() {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function (position) {
            console.log('>>>>> curreunt position : ' + position.coords.latitude + ', ' + position.coords.longitude);
        ...
        currentInfowindow.setContent('<div style="padding:20px;"><h5 style="margin-bottom:5px;color:#f00;">Geolocation not supported</h5></div>');
...
function setUserMarkers(latitude, longitude) {
        ...
        for (var i = 0; i < 3; i++) {
            latitude = latitude + (i * 0.001 + (Math.random() / 1000));
            longitude = longitude + (i * 0.001 + (Math.random() / 1000));
            ...
            var userMarker = new naver.maps.Marker(
                {
                    ...
                    icon: {
                        ...
                        url: 'https://cdn-icons-png.flaticon.com/128/9512/9512683.png',  // 사람
                        scaledSize: new naver.maps.Size(26, 26),  // 표시할 크기 설정
                        origin: new naver.maps.Point(0, 0),
                        anchor: new naver.maps.Point(13, 13)  // 아이콘 중심점 설정
...
function clickUserMarker(e) {
    ...
    console.log(e);
    sendMessageToApp(MySendCommands.openCarInfo, 'null');
...
function movePosition(lat, lng) {
    ...
    myCarMarker.setPosition(new naver.maps.LatLng(lat, lng));
    myCarMarker.setMap(map);
    
    naver.maps.Event.addListener(myCarMarker, 'click', clickUserMarker);
...
// setInterval(() => {
//     console.log(myCarMarker.getPosition());
//     latitude = myCarMarker.getPosition().lat() + 0.0001;
//     longitude = myCarMarker.getPosition().lng() + 0.0001;
//     console.log('>>> latitude : ' + latitude);  
//     console.log('>>> longitude : ' + longitude);
//     myCarMarker.setPosition(new naver.maps.LatLng(latitude, longitude));
// }, 3000);